...
export default function buildPreset(context, opts = {}) {
    ...
    let buildBrowserList = options.target === 'browser' || options.target === 'web';
    let buildLibrary = options.target === 'library' || options.target === 'es2015' || options.target === 'modern';
    ...
    if (buildBinary) {
        ...
        const autoBrowsers = browserlist(null, { env: isProd ? 'production' : 'development' });
    ...
    const requiresAsync = isPluginRequired(getTargets(envTargets), envPlugins['transform-async-to-generator']);
    if (!requiresAsync) {
    ...
    if (options.rewriteAsync === 'promises') {
        plugins.push([
            fastAsyncPlugin, { useRuntimeModule: true }
        ]);
    }
